...
- 软件更新完成后，必须同步源码、创建对应版本标签和 GitHub Release，并上传该版本 APK，不能只推送代码。
- 使用中文填写每个版本的发布说明，写清相对上一版的新增功能、功能调整、问题修复、验证结果、已知限制，以及必要的升级和备份提示；不适用项可省略。
- 更新 Android versionCode / versionName，并保持版本标签、发布标题及 APK 文件名一致；保留历史版本和附件，不用新包覆盖旧包。
- 发布前执行相应测试与构建，保留现有签名以支持覆盖安装；发布后核对 APK 大小与 SHA-256，提供该版本直接下载链接和最新版本入口。
- 最新版本入口：https://github.com/wfeng5830-lab/beiwang-licai/releases/latest 。测试或预发布版本应明确标识。
- 仅修改文档或项目管理规则时，无需生成新的软件版本或 APK。
